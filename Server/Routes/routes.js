...
router.post("/init/brands", async (req, res) => {
  tokenUtilities.validateTokenAndIfAdmin(
    ...
    async (success, result) => {
      ...
      brandController.fetchBrandByName(
        "Indefinido",
        (fetchBrandSuccess, fetchBrandResult) => {
          if (fetchBrandSuccess) {
            ...
            brandController.initializeBrandModel(
              ...
              (initSuccess, initBrandResult) => {
                res
                  .status(initBrandResult.processRespCode)
                  .send(initBrandResult.toClient);
              }
            );
          } else {
            res
              .status(fetchBrandResult.processRespCode)
              .send(initBrandResult.toClient);
          }
        }
      );
          ...
          if (fetchBrandSuccess) {
            ...
            brandController.addBrand(
              ...
              (addBrandSuccess, addBrandResult) => {
                res
                  .status(addBrandResult.processRespCode)
                  .send(addBrandResult.toClient);
              }
            );
          } else {
            res
              .status(fetchBrandResult.processRespCode)
              .send(fetchBrandResult.toClient);
          }
        }
      );
    }
          ...
          if (fetchBrandSuccess) {
            ...
            brandController.updateBrand(
              ...
              (updateBrandSuccess, updateBrandResult) => {
                res
                  .status(updateBrandResult.processRespCode)
                  .send(updateBrandResult.toClient);
              }
            );
          } else {
            res
...
router.delete("/engines/:id", async (req, res) => {
  tokenUtilities.validateTokenAndIfAdmin(
    ...
    async (success, result) => {
      ...
      carModelController.fetchCarModelByBrandId(
          ...
          if (fetchSuccess) {
            ...
            brandController.deleteBrand(
              ...
              (deleteSuccess, deleteResult) => {
                res
                  .status(deleteResult.processRespCode)
                  .send(deleteResult.toClient);
              }
            );
          } else {
...
router.post("/init/engines", async (req, res) => {
  tokenUtilities.validateTokenAndIfAdmin(
    ...
    async (success, result) => {
      ...
      let fetchConfirmExist = false;
      engineController.fetchEngineByName(
        "Indefinido",
        (fetchEngineSuccess, fetchEngineResult) => {
          if (fetchEngineSuccess) {
            ...
            engineController.initializeEngineModel(
              ...
              (initSuccess, initEngineResult) => {
                res
                  .status(initEngineResult.processRespCode)
                  .send(initEngineResult.toClient);
              }
            );
          } else {
            res
              .status(fetchEngineResult.processRespCode)
              .send(fetchEngineResult.toClient);
          }
        }
      );
    }
            ...
            engineController.addEngine(
              {
                ...
                horsePower: req.sanitize(req.body.horsePower)
                  ? req.sanitize(req.body.horsePower)
                  : null
              },
              (addEngineSuccess, addEngineResult) => {
                res
                  .status(addEngineResult.processRespCode)
                  .send(addEngineResult.toClient);
              }
            );
          } else {
            res
      ...
      engineController.fetchEngineByName(
        ...
        (fetchEngineSuccess, fetchEngineResult) => {
          if (fetchEngineSuccess) {
            ...
            engineController.fetchEngineById(
              req.sanitize(req.params.id),
              (fetchEngineByIdSuccess, fetchEngineByIdResult) => {
                if (fetchEngineByIdSuccess) {
                  if (fetchEngineByIdResult.processRespCode !== 200) {
                    if (
                      fetchEngineByIdResult.toClient.processResult[0]
                  ...
                  engineController.updateEngine(
                    ...
                    (updateEngineSuccess, updateEngineResult) => {
                      res
                        .status(updateEngineResult.processRespCode)
                        .send(updateEngineResult.toClient);
                    }
                  );
                } else {
                  res
                    .status(fetchEngineByIdResult.processRespCode)
                    .send(fetchEngineByIdResult.toClient);
                }
              }
            );
          } else {
...
router.delete("/engines/:id", async (req, res) => {
  tokenUtilities.validateTokenAndIfAdmin(
    ...
    async (success, result) => {
      ...
      carController.fetchCarByEngineId(
        req.sanitize(req.params.id),
        (fetchSuccess, fetchResult) => {
          if (fetchSuccess) {
            if (fetchResult.processRespCode === 200) {
...
router.post("/init/fuels", async (req, res) => {
  tokenUtilities.validateTokenAndIfAdmin(
    ...
    async (success, result) => {
      ...
      let fetchConfirmExist = false;
      fuelController.fetchFuelByName(
        "Indefinido",
        (fetchFuelSuccess, fetchFuelResult) => {
          if (fetchFuelSuccess) {
            ...
            fuelController.initializeFuelModel(
              ...
              (initSuccess, initFuelResult) => {
                res
                  .status(initFuelResult.processRespCode)
                  .send(initFuelResult.toClient);
              }
            );
          } else {
            res
              .status(fetchFuelResult.processRespCode)
              .send(fetchFuelResult.toClient);
          }
        }
      );
    }
          ...
          if (fetchFuelSuccess) {
            ...
            fuelController.addFuel(
              ...
              (addFuelSuccess, addFuelResult) => {
                res
                  .status(addFuelResult.processRespCode)
                  .send(addFuelResult.toClient);
              }
            );
          } else {
            res
      ...
      fuelController.fetchFuelByName(
        ...
        (fetchFuelSuccess, fetchFuelResult) => {
          if (fetchFuelSuccess) {
            ...
            fuelController.fetchFuelById(
              req.sanitize(req.params.id),
              (fetchFuelByIdSuccess, fetchFuelByIdResult) => {
                if (fetchFuelByIdSuccess) {
                  if (fetchFuelByIdResult.processRespCode !== 200) {
                    if (
                      fetchFuelByIdResult.toClient.processResult[0]
                        .description === "Indefinido"
                    ) {
                      canBeEdited = false;
                    }
                  ...
                  fuelController.updateFuel(
                    ...
                    (updateFuelSuccess, updateFuelResult) => {
                      res
                        .status(updateFuelResult.processRespCode)
                        .send(updateFuelResult.toClient);
                    }
                  );
                } else {
                  res
                    .status(fetchFuelByIdResult.processRespCode)
                    .send(fetchFuelByIdResult.toClient);
                }
              }
            );
          } else {
...
router.delete("/fuels/:id", async (req, res) => {
  tokenUtilities.validateTokenAndIfAdmin(
    ...
    async (success, result) => {
      ...
      let fetchConfirmCarExist = false;
      carController.fetchCarByFuelId(
        req.sanitize(req.params.id),
        (fetchSuccess, fetchResult) => {
          if (fetchSuccess) {
            if (fetchResult.processRespCode === 200) {
...
router.post("/models", async (req, res) => {
  tokenUtilities.validateTokenAndIfAdmin(
    ...
    async (success, result) => {
      ...
      let fetchConfirmExist = false;
      carModelController.fetchModelByName(
        req.sanitize(req.body.designation),
          ...
          if (fetchModelSuccess) {
            ...
            carModelController.addModel(
              ...
              (addModelSuccess, addModelResult) => {
                res
                  .status(addModelResult.processRespCode)
                  .send(addModelResult.toClient);
              }
            );
          } else {
            res
              .status(fetchModelResult.processRespCode)
              .send(fetchModelResult.toClient);
          }
        }
      );
    }
          ...
          if (fetchModelSuccess) {
            ...
            carModelController.updateModel(
              ...
              (updateModelSuccess, updateModelResult) => {
                res
                  .status(updateModelResult.processRespCode)
                  .send(updateModelResult.toClient);
              }
            );
          } else {
            res
...
router.delete("/models/:id", async (req, res) => {
  tokenUtilities.validateTokenAndIfAdmin(
    ...
    async (success, result) => {
      ...
      let fetchConfirmCarExist = false;
      carController.fetchCarByModelId(
        req.sanitize(req.params.id),
        (fetchSuccess, fetchResult) => {
          if (fetchSuccess) {
            if (fetchResult.processRespCode === 200) {
...
router.post("/cars", async (req, res) => {
  tokenUtilities.validateTokenAndIfAdmin(
    ...
    async (success, result) => {
      ...
      if (
        (req.body.regPlate === null,
        req.body.facebookUrl === null,
        req.body.idEngineType === null,
        req.body.idModel === null,
        req.body.idFuelType === null,
        req.body.price === null,
      ...
      await carController.confirmExistence(
        ...
        (confirmSuccess, confirmResult) => {
          ...
          if (!confirmSuccess) {
            res
              .status(confirmResult.processRespCode)
              .send(confirmResult.toClient);
          }
          carController.addCar(
            ...
            (addSuccess, addResponse) => {
              res
                .status(addResponse.processRespCode)
                .send(addResponse.toClient);
            }
          );
        }
      );
        ...
        (confirmSuccess, confirmResult) => {
          ...
          carController.updateCar(
            ...
            (updateSuccess, updateResponse) => {
              res
                .status(updateResponse.processRespCode)
                .send(updateResponse.toClient);
            }
          );
        }
      );
      ...
      await carController.fetchCars(
        ...
        (fetchSuccess, fetchResponse) => {
          if (!fetchSuccess || fetchResponse.processRespCode === 204) {
            res
              .status(fetchResponse.processRespCode)
              .send(fetchResponse.toClient);
          }
          carController.fetchCarsImgByPath(
            ...
            (fetchImgSuccess, fetchImgResponse) => {
              res
                .status(fetchImgResponse.processRespCode)
                .json(fetchImgResponse.toClient);
            }
          );
        }
      );
...
router.get("/testimonials", async (req, res) => {
  testimonialController.fetchTestimonials(
    req,
    (fetchSuccess, fetchResponse) => {
      res.status(fetchResponse.processRespCode).json(fetchResponse.toClient);
    }
    ...
    async (success, result) => {
      ...
      testimonialController.deleteTestimonial(
        ...
        (deleteSuccess, deleteResponse) => {
          res
            .status(deleteResponse.processRespCode)
            .send(deleteResponse.toClient);
        }
      );
    }
  );
