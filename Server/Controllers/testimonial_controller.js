  ...
  sequelize
    ...
    .then(data => {
      ...
      if (data[0].length === 0) {
        respCode = 204;
        respMsg =
          "Fetch process completed successfully, but there is no content.";
      }
      processResp = {
...
addTestimonial = (dataObj, callback) => {
  ...
  if (
    dataObj.req.body.personName === null ||
    dataObj.req.body.testimonialText === null
  ) {
    processResp = {
      processRespCode: 409,
      toClient: {
    ...
    .query(
      "UPDATE fuel SET person_name = :person_name,testimonial_text=:testimonial_text Where testimonial.id_testimonial = :id_testimonial;",
      {
        replacements: {
          ...
          testimonial_text: dataObj.req.sanitize(
            dataObj.req.body.testimonialText
          ),
